  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f4f6fb', padding: '2rem 1rem', fontFamily: 'sans-serif' }}>
      <div style={{ maxWidth: '720px', margin: '0 auto' }}>
        ...
        {loading && <p style={{ textAlign: 'center', color: '#888', fontSize: '14px' }}>Loading...</p>}
        ...
        {emails.map(email => (
          <div key={email._id} style={{ background: '#fff', borderRadius: '12px', padding: '1.5rem', marginBottom: '1rem', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
              ...
              <span style={{ padding: '4px 12px', borderRadius: '100px', fontSize: '12px', fontWeight: '600', background: email.status === 'sent' ? '#e8f5e9' : '#ffebee', color: email.status === 'sent' ? '#2e7d32' : '#c62828' }}>
                {email.status === 'sent' ? '✅ Sent' : '❌ Failed'}
              </span>
            </div>
            <p style={{ margin: '0 0 8px', fontSize: '13px', color: '#666' }}><strong>To:</strong> {email.recipients.join(', ')}</p>
            <p style={{ margin: '0 0 8px', fontSize: '13px', color: '#888', borderTop: '1px solid #f0f0f0', paddingTop: '8px' }}>{email.body?.substring(0, 120)}{email.body?.length > 120 ? '...' : ''}</p>
            <p style={{ margin: 0, fontSize: '12px', color: '#bbb' }}>{new Date(email.sentAt).toLocaleString()}</p>
          </div>
        ))}
      </div>
    </div>
